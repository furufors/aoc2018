...
main :: IO ()
main = interact $ show . strategy2 . buildStatistics . groupByShift . map parsein . lines

strategy2 :: GuardStatistics -> Int
strategy2 gs = bestMinute * guardId
    where
        (guardId,bestMinute,_) = head . reverse . sortBy (comparing (\(a,b,c) -> c)) $ gmc
        gmc = concat $ map countSleepTime . M.assocs $ gs
        countSleepTime :: (Guard,[(UTCTime, NightShift)]) -> [(Guard,Int,Int)]
        countSleepTime (g,tns) =
            let sleepCounts = zip [0..59] $ foldl' sleepCount (take 60 $ repeat 0) . map snd $ tns
            in map (\(m,c) -> (g,m,c)) sleepCounts
